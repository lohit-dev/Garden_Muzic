  ...
  run: async (client, interaction, data) => {
    ...
    if (data2) {
      if (data2.Mode) {
        ...
        if (!pass && !interaction.member.permissions.has(Permissions.FLAGS.MANAGE_GUILD))
          return await buttonReply(
            interaction,
            "You don't have dj role to use this command",
            color
          );
      }
    }
    ...
    if (interaction.customId === `${interaction.guildId}pause`) {
      ...
      // Use skip method from Kazagumo instead of stopTrack
      await player.skip();
      if (message)
        await message
          .edit({
      ...
      if (amount <= 10)
        return await buttonReply(interaction, 'Volume Cannot Decrease Below `[ 10% ]`.', color);
      if (message)
        await message
          .edit({
            embeds: [nowplaying],
          ...
          .catch(() => {});
      // Kazagumo uses 0-100 scale for volume
      await player.setVolume(amount);
      await buttonReply(
        interaction,
        `${volumeEmoji} Volume set to: \`[ ${player.volume}% ]\``,
        color
      );
      if (message)
        await message
      ...
      let amount = Number(player.volume + 10);
      if (amount >= 100)
        return await buttonReply(interaction, 'Volume Cannot Exceed `[ 100% ]`', color);
      // Kazagumo uses 0-100 scale for volume
      await player.setVolume(amount);
      await buttonReply(
        interaction,
