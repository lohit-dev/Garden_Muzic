  ...
  run: async (client, id) => {
    ...
    setTimeout(async () => {
      try {
        ...
        client.logger.log(
          `Checking ${guildsInShard.size} guilds in shard #${id} for default playlists`,
          'log'
        );
        ...
        for (const [guildId, guild] of guildsInShard) {
          try {
            ...
            const defaultPlaylist = await defaultPlaylistDb.findOne({
              Guild: guildId,
              Enabled: true,
            });
            ...
            if (result && result.tracks && result.tracks.length) {
              ...
              client.logger.log(
                `Started default playlist in ${guild.name} after shard reconnect`,
                'log'
              );
            }
          } catch (error) {
            client.logger.log(
              `Error starting default playlist for ${guildId} after shard reconnect: ${error.message}`,
              'error'
            );
          }
        }
        ...
        client.logger.log(
          `Error handling shard reconnect for default playlists: ${error.message}`,
          'error'
        );
      }
