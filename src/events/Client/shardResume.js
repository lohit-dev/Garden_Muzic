  ...
  run: async (client, id, replayedEvents) => {
    ...
    setTimeout(async () => {
      try {
        ...
        for (const [guildId, guild] of guildsInShard) {
          try {
            ...
            if (result && result.tracks && result.tracks.length) {
              ...
              client.logger.log(
                `Started default playlist in ${guild.name} after shard resume`,
                'log'
              );
            }
          } catch (error) {
            client.logger.log(
              `Error starting default playlist for ${guildId} after shard resume: ${error.message}`,
              'error'
            );
          }
        }
        ...
        client.logger.log(
          `Error handling shard resume for default playlists: ${error.message}`,
          'error'
        );
      }
