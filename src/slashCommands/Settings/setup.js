...
module.exports = {
  ...
  run: async (client, interaction, prefix) => {
    ...
    if (interaction.options.getSubcommand() === 'set') {
      ...
      const title =
        player && player.queue && player.current ? 'Now playing' : 'Nothing is playing right now';
      const desc =
      ...
      const footer = {
        text:
          player && player.queue && player.current
            ? `Requested by ${player.current.requester.tag}`
            : '',
        iconURL:
          player && player.queue && player.current
            ? `${player.current.requester.displayAvatarURL({ dynamic: true })}`
      ...
      const but1 = new MessageButton()
        .setCustomId(`${interaction.guildId}pause`)
        .setEmoji('⏸️')
        .setStyle('SECONDARY')
        .setDisabled(disabled);
      const but2 = new MessageButton()
        .setCustomId(`${interaction.guildId}previous`)
        .setEmoji('⏮️')
        .setStyle('SECONDARY')
        .setDisabled(disabled);
      const but3 = new MessageButton()
        .setCustomId(`${interaction.guildId}skip`)
        .setEmoji('⏭️')
        .setStyle('SECONDARY')
        .setDisabled(disabled);
      const but4 = new MessageButton()
        .setCustomId(`${interaction.guildId}voldown`)
        .setEmoji('🔉')
        .setStyle('SECONDARY')
        .setDisabled(disabled);
      const but5 = new MessageButton()
        .setCustomId(`${interaction.guildId}volup`)
        .setEmoji('🔊')
        .setStyle('SECONDARY')
        .setDisabled(disabled);
      ...
      if (!data)
        return await interaction.reply({
          content:
            "This server doesn't have any song request channel setup to use this sub command.",
        });
      await data.delete();
