    ...
    return message.channel.send({
      embeds: [
        new MessageEmbed()
          ...
          .setDescription('Unable to autoplay from the previous track. Destroyed the player.'),
      ],
    });
  }
  const result = await player.search(searched, requester);
  if (result.tracks.length > 1) {
    await player.queue.add(result.tracks[1]);
    if (result.tracks.length > 2) {
      await player.queue.add(result.tracks[2]);
    }
  }
  return player.play();
}
...
async function playerhandler(query, player, message) {
  ...
  return message.channel
    ...
    .then(a => setTimeout(async () => await a.delete().catch(() => {}), 5000))
    .catch(() => {});
}
  ...
  try {
    ...
    if (!message) {
      let embed1 = new MessageEmbed()
        .setColor(client.embedColor)
        .setDescription(
          `[${track.title}](${track.uri}) - \`[ ${track.isStream ? '[**◉ LIVE**]' : convertTime(player.current.length)} ]\``
        )
        .setImage(icon)
        .setFooter({
          text: `Requested by ${player.current.requester.tag}`,
      ...
      const but1 = new MessageButton()
        .setCustomId(`${player.guild}pause`)
        .setEmoji('⏸️')
        .setStyle('SECONDARY');
      const but2 = new MessageButton()
        .setCustomId(`${player.guild}previous`)
        .setEmoji('⏮️')
        .setStyle('SECONDARY');
      const but3 = new MessageButton()
        .setCustomId(`${player.guild}skip`)
        .setEmoji('⏭️')
        .setStyle('SECONDARY');
      const but4 = new MessageButton()
        .setCustomId(`${player.guild}voldown`)
        .setEmoji('🔉')
        .setStyle('SECONDARY');
      const but5 = new MessageButton()
        .setCustomId(`${player.guild}volup`)
        .setEmoji('🔊')
        .setStyle('SECONDARY');
